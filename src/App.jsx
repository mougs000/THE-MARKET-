...
export default function TheMarket() {
  ...
  const publishListing = async (draft, actingForSeller) => {
    ...
    const p = {
      id: uid(), ...draft, seller: identity.name, sellerType: identity.sellerType,
      verified: identity.verified || "Unverified", createdAt: Date.now(), status: "active",
      sellerBankDetails: identity.bankDetails || null,
  ...
  return (
    <div>
        ...
        <div className="p-3 rounded-xl mb-3" style={{ background: "#fff", border: `1px solid ${COLORS.line}` }}>
          ...
          {!bankComplete && (
            <div className="text-xs mt-2 p-2 rounded-lg" style={{ background: `${COLORS.rust}18`, color: COLORS.rust }}>
              {actingFor ? `${actingFor.name} hasn't added payout bank details yet — they need to add these in their own Account before this can publish.` : "You haven't added payout bank details yet — add them in Account first, or buyers won't have a way to pay you."}
            </div>
          )}
          {!sellerDetailsComplete && (
            <div className="text-xs mt-2 p-2 rounded-lg" style={{ background: `${COLORS.rust}18`, color: COLORS.rust }}>
              {actingFor ? `${actingFor.name}'s seller details aren't complete yet — they need to add these in their own Account.` : `Your ${profile.role === "business" ? "business" : "seller"} details aren't complete yet — add them in Account first.`}
            </div>
          )}
        </div>
      )}
      ...
      <div className="flex gap-2 mt-4">
        ...
        {step === steps.length - 1 && (
          <Btn full variant="lime" disabled={!bankComplete || !sellerDetailsComplete} onClick={() => {
            onPublish({ ...d, price: Number(d.price), deliveryFee: Number(d.deliveryFee) || 0, qty: Number(d.qty) || 1 }, actingFor);
          }}>
            {!bankComplete ? "Payout details missing" : !sellerDetailsComplete ? "Seller details missing" : actingFor ? `Publish for ${actingFor.name}` : "Publish listing"}
          </Btn>
        )}
      </div>
    </div>
