...
export default function TheMarket() {
  ...
  const persistCart = async (next) => { setCart(next); await savePersonal("cart", next); };

  const saveProfile = async (p) => { setProfile(p); await savePersonal("profile", p); };
...
/* ---------------- ACCOUNT ---------------- */
function AccountView({ profile, nameInput, setNameInput, onSave, messages, products, respondOffer, onShowAdmin }) {
      ...
      {!profile ? (
        <div className="p-4 rounded-xl" style={{ background: "#fff", border: `1px solid ${COLORS.line}` }}>
          ...
          <div className="mt-2 flex gap-2">
            <Btn variant="lime" full onClick={() => nameInput.trim() && onSave({ name: nameInput.trim(), role: "individual", verified: "Unverified" })}>Create account</Btn>
          </div>
        </div>
      ) : (
        <div>
