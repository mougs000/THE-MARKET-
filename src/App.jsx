...
export default function TheMarket() {
  ...
  const [settings, setSettings] = useState({ commissionRate: 0.01, bankDetails: DEFAULT_BANK_DETAILS });
  ...
  useEffect(() => {
    ...
    (async () => {
      const [p, o, m, r, s, prof, c, sup] = await Promise.all([
        ...
        loadShared("messages", []),
        loadShared("reviews", []),
        loadShared("settings", { commissionRate: 0.01, bankDetails: DEFAULT_BANK_DETAILS }),
      ...
      setSettings({ commissionRate: 0.01, bankDetails: DEFAULT_BANK_DETAILS, ...s });
      setSupport(sup);
      ...
      <div className="px-4 pb-28 pt-4">
        ...
        {tab === "orders" && (
          <OrdersView buyerOrders={myOrdersAsBuyer} sellerOrders={myOrdersAsSeller}
            ...
            onConfirmPayment={confirmPayment} bankDetails={settings.bankDetails || DEFAULT_BANK_DETAILS} />
        )}
        {tab === "account" && (
...
function SellView({ onPublish, profile, onNeedProfile }) {
  ...
  const handleVideoUpload = async (e) => {
    ...
    if (file.size > MAX_MB * 1024 * 1024) {
      setVideoError(`That video is ${(file.size / 1024 / 1024).toFixed(0)}MB — please keep it under ${MAX_MB}MB. Try a shorter clip (10-15 sec) or lower the recording quality in your camera settings, then try again.`);
    ...
    try {
      ...
      const path = `${Date.now()}-${Math.random().toString(36).slice(2, 8)}-${safeName}`;
      const { error } = await client.storage.from("product-images").upload(path, file);
      if (error) { setVideoError("Upload failed — check your connection and try again."); console.error(error); }
      else {
        const { data } = client.storage.from("product-images").getPublicUrl(path);
          ...
          <Field label="Product video (optional)">
            ...
            {d.videoUrl ? (
              ...
              <Btn small variant="ghost" disabled={uploadingVideo} onClick={() => videoInputRef.current?.click()}>
                {uploadingVideo ? "Uploading… this can take a minute on mobile data" : "🎥 Choose video"}
              </Btn>
            )}
            {videoError && <div className="text-xs mt-1" style={{ color: COLORS.rust }}>{videoError}</div>}
            <div className="text-[11px] opacity-45 mt-1">Under 20MB. Keep clips short (10-15 sec) — WiFi uploads much faster than mobile data.</div>
          </Field>
...
function CartView({ items, onRemove, onCheckout, settings }) {
  ...
  const bank = settings.bankDetails || DEFAULT_BANK_DETAILS;
    ...
    return (
      <div>
        ...
        <div className="p-4 rounded-xl mb-3" style={{ background: "#fff", border: `1px solid ${COLORS.line}` }}>
          <div className="text-xs opacity-60 mb-3">Make the transfer using these details, then confirm below. Your order will show as "awaiting payment confirmation" until the seller checks it landed.</div>
          ...
          <div style={{ borderTop: `1px solid ${COLORS.line}` }} className="pt-2 mt-2">
            <Row label="Amount" value={money(total, items[0].product.currency)} bold />
          </div>
        </div>
        <Btn full variant="lime" onClick={() => onCheckout(items, delivery)}>I've made the EFT — place order</Btn>
      </div>
    );
  }
...
/* ---------------- ORDERS ---------------- */
function OrdersView({ buyerOrders, sellerOrders, onAdvance, onReview, reviews, profile, onConfirmPayment, bankDetails }) {
        ...
        {list.map(o => {
          ...
          return (
            <div key={o.id} className="p-3 rounded-xl" style={{ background: "#fff", border: `1px solid ${COLORS.line}` }}>
              ...
              {view === "buying" && o.paymentMethod === "eft" && !o.paymentConfirmed && bankDetails && (
                <div className="p-2 rounded-lg text-[11px] mb-2" style={{ background: COLORS.paper2 }}>
                  EFT to {bankDetails.bankName} · {bankDetails.accountName} · {bankDetails.accountNumber}
                </div>
              )}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end" style={{ background: "#00000066" }}>
      <div className="w-full rounded-t-2xl p-4 max-h-[85vh] overflow-y-auto" style={{ background: COLORS.paper }}>
        ...
        <SectionTitle>EFT payout details (shown to buyers at checkout)</SectionTitle>
        <div className="space-y-2 mb-2">
          ...
          <Btn small variant="lime" onClick={() => onSaveSettings({ ...settings, bankDetails: bank })}>Save bank details</Btn>
        </div>
        <div className="text-[11px] opacity-50 mb-4">Every buyer checking out sees these details to make an EFT. Swap in a real payment gateway later without changing this screen.</div>
