...
/* ---------------- PRODUCT ---------------- */
function ProductView({ product: p, onBack, onAddCart, onBuyNow, onMessage, onOffer, onRespondOffer, messages, profile, reviews, settings, follows, onToggleFollow, session }) {
    ...
    <div>
      ...
      <div className="p-3 rounded-xl mb-3" style={{ background: "#fff", border: `1px solid ${COLORS.line}` }}>
        ...
        <div className="flex items-center justify-between mt-2">
          <div className="text-[11px] opacity-60">{followerCount} follower{followerCount !== 1 ? "s" : ""}</div>
          {profile?.name !== p.seller && (
            <Btn small variant={isFollowing ? "outline" : "lime"} onClick={() => onToggleFollow(p.seller)}>{isFollowing ? "Following ✓" : "Follow"}</Btn>
          )}
        </div>
        <div className="mt-2"><DirectLine small /></div>
      </div>
