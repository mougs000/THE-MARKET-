  ...
  return (
    <div>
      ...
      <div className="flex gap-2 mt-4">
        ...
        {step === steps.length - 1 && (
          <Btn full variant="lime" disabled={!profile.bankDetails?.accountNumber} onClick={() => {
            onPublish({ ...d, price: Number(d.price), deliveryFee: Number(d.deliveryFee) || 0, qty: Number(d.qty) || 1 });
          }}>
            {profile.bankDetails?.accountNumber ? "Publish listing" : "Add payout details first"}
          </Btn>
        )}
      </div>
    </div>
...
/* ---------------- ORDERS ---------------- */
function OrdersView({ buyerOrders, sellerOrders, onAdvance, onReview, reviews, profile, onConfirmPayment }) {
