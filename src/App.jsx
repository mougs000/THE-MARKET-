...
// App-level admin restriction. This is a UI-level gate, not database-enforced —
// a technically determined user could bypass it via dev tools, since it isn't
// backed by a Supabase RLS policy. It closes the obvious gap (anyone tapping the
// button) but isn't a substitute for a real server-side admin role if that matters later.
const ADMIN_EMAILS = ["kingmorgazi1@gmail.com"];
...
function Logo({ size = 30 }) {
  return (
    <img src="/logo.png" alt="THE MARKET" width={size} height={size}
      style={{ flexShrink: 0, borderRadius: size * 0.28, objectFit: "cover" }} />
  );
}
...
function NavIcon({ id, size = 22, color = COLORS.ink }) {
  const s = { stroke: color, strokeWidth: 2, fill: "none", strokeLinejoin: "round", strokeLinecap: "round" };
  const paths = {
    home: <g><path d="M4 12 L13 4 L22 12" {...s} /><path d="M6.5 10.5 V21 H19.5 V10.5" {...s} /><rect x="10.5" y="14" width="5" height="7" fill={color} /></g>,
    categories: <g><circle cx="11.5" cy="11.5" r="7.5" {...s} /><path d="M17 17 L22.5 22.5" {...s} /></g>,
    orders: <g><path d="M4 8 L13 3.5 L22 8 L13 12.5 Z" {...s} /><path d="M4 8 V18 L13 22.5 L22 18 V8" {...s} /><path d="M13 12.5 V22.5" {...s} /></g>,
    account: <g><circle cx="13" cy="8.5" r="4.5" {...s} /><path d="M4 22 C4 16.5 8 14 13 14 C18 14 22 16.5 22 22" {...s} /></g>,
  };
  return <svg width={size} height={size} viewBox="0 0 26 26">{paths[id]}</svg>;
}
...
function AuthView({ onClose }) {
  ...
  return (
    <div style={{ background: `linear-gradient(160deg, ${COLORS.ink} 0%, ${COLORS.tealDark} 55%, ${COLORS.teal} 100%)`, minHeight: "100vh" }}
      ...
      <div className="relative flex flex-col items-center mb-8 w-full max-w-sm mx-auto">
      ...
      <div className="relative p-5 rounded-2xl w-full max-w-sm mx-auto" style={{ background: COLORS.paper, boxShadow: "0 20px 50px rgba(0,0,0,0.35)" }}>
...
export default function TheMarket() {
  ...
  const [actingForSeller, setActingForSeller] = useState(null);
  ...
  // and refetches whenever the tab regains focus so people see new listings
  ...
  useEffect(() => {
    const onFocus = () => loadPublicData();
    const onVisible = () => { if (document.visibilityState === "visible") loadPublicData(); };
  ...
  const saveProfile = async (p) => {
    setProfile(p);
    if (!session) return;
    await savePersonal(`profile:${session.user.id}`, p);
    // Keep a lightweight, non-primary directory entry so an admin can pick this
    // seller when listing on their behalf — never overwrites another seller's entry.
    if (p?.name) {
      const dir = (await loadShared("sellers", [])) || [];
      const entry = {
        userId: session.user.id, name: p.name, sellerType: p.role === "business" ? "Business" : "Individual",
        verified: p.verified || "Unverified", sellerDetails: p.sellerDetails || null,
        bankDetails: p.bankDetails || null, premiumUntil: p.premiumUntil || null, updatedAt: Date.now(),
      };
      const next = [...dir.filter(d => d.userId !== session.user.id), entry];
      await saveShared("sellers", next);
    }
  };
  const loadSellerDirectory = async () => setSellers((await loadShared("sellers", [])) || []);
  ...
  const placeOrder = async (items, deliveryChoice) => {
    ...
    const newOrders = items.map(ci => {
      ...
      return {
        ...
        paymentMethod: p.sellerBankDetails?.accountNumber ? "eft" : "contact", paymentConfirmed: false, paymentReference: reference,
        commissionPaid: false,
        sellerBankDetails: p.sellerBankDetails || null,
        sellerContactPhone: p.sellerContactPhone || null,
      };
    });
  ...
  const publishListing = async (draft, actingForSeller) => {
    if (!ensureProfile()) return;
    const identity = actingForSeller || {
      name: profile.name, sellerType: profile.role === "business" ? "Business" : "Individual",
      verified: profile.verified || "Unverified", bankDetails: profile.bankDetails, premiumUntil: profile.premiumUntil,
    };
    const { contactPhone, ...rest } = draft;
    const p = {
      id: uid(), ...rest, seller: identity.name, sellerType: identity.sellerType,
      verified: identity.verified || "Unverified", createdAt: Date.now(), status: "active",
      sellerBankDetails: identity.bankDetails?.accountNumber ? identity.bankDetails : null,
      sellerContactPhone: !identity.bankDetails?.accountNumber && contactPhone?.trim() ? contactPhone.trim() : null,
      sellerPremiumUntil: identity.premiumUntil || null,
      listedByAdmin: actingForSeller ? profile.name : null,
    };
    const latest = (await loadShared("products", products)) || products;
    await persistProducts([p, ...latest]);
    flash(actingForSeller ? `Listing published for ${identity.name}` : "Listing published");
    if (actingForSeller) setActingForSeller(null);
    setTab("home");
  };

  const markListingSold = async (productId) => {
    const latest = (await loadShared("products", products)) || products;
    await persistProducts(latest.map(p => p.id === productId ? { ...p, status: "sold" } : p));
    flash("Marked as sold");
  };

  const deleteListing = async (productId) => {
    const latest = (await loadShared("products", products)) || products;
    await persistProducts(latest.filter(p => p.id !== productId));
    flash("Listing deleted");
  };

  const relistListing = async (productId) => {
    const latest = (await loadShared("products", products)) || products;
    await persistProducts(latest.map(p => p.id === productId ? { ...p, status: "active" } : p));
  ...
  return (
    <div className="min-h-screen md:flex md:items-center md:justify-center md:py-6"
      style={{ background: `radial-gradient(circle at 50% 0%, ${COLORS.tealDark}22, ${COLORS.ink} 70%)` }}>
      <div style={{ background: COLORS.paper, minHeight: "100vh", fontFamily: "'Inter', sans-serif", color: COLORS.ink, transform: "translateZ(0)" }}
        className="w-full md:max-w-[480px] md:min-h-[calc(100vh-48px)] md:rounded-3xl md:shadow-2xl md:overflow-hidden relative">
      <style>{FONT_IMPORT}</style>
      ...
      <div className="px-4 pb-28 pt-4">
        ...
        {tab === "sell" && (
          <SellView onPublish={publishListing} profile={profile} onNeedProfile={() => setTab("account")}
            actingFor={actingForSeller} onExitActingFor={() => setActingForSeller(null)} />
        )}
        {tab === "cart" && (
      ...
      {showAdmin && ADMIN_EMAILS.includes(session?.user?.email) && (
        <AdminModal onClose={() => setShowAdmin(false)} products={products} orders={orders} settings={settings}
          ...
          sellers={sellers} onLoadSellers={loadSellerDirectory}
          onListForSeller={(sellerEntry) => { setActingForSeller(sellerEntry); setShowAdmin(false); setTab("sell"); }} />
      )}
        ...
        <div className="flex items-end justify-around px-1 pt-2 pb-2 relative">
          {[
            ...
            <button key={t.id} onClick={() => setTab(t.id)}
              ...
              style={tab === t.id ? { background: `${COLORS.teal}14` } : {}}>
              <NavIcon id={t.id} size={21} color={tab === t.id ? COLORS.teal : COLORS.ink} />
              <span className="text-[10px] font-semibold" style={{ color: tab === t.id ? COLORS.teal : COLORS.ink, opacity: tab === t.id ? 1 : 0.5 }}>{t.label}</span>
            </button>
          ))}

          <button onClick={() => (!session) ? setShowAuthPrompt(true) : setTab("sell")}
            className="flex flex-col items-center gap-1 transition active:scale-95"
            style={{ marginTop: -34 }}>
            <span className="flex items-center justify-center rounded-full"
              style={{
                width: 62, height: 62,
                background: `linear-gradient(135deg, ${COLORS.lime}, #c2ee2e)`,
                boxShadow: `0 6px 18px ${COLORS.lime}80, 0 0 0 5px #fff`,
                fontSize: 30, color: COLORS.ink, fontWeight: 700,
              }}>＋</span>
            <span className="text-[10px] font-bold" style={{ color: COLORS.teal }}>Sell</span>
          </button>

          {[
            { id: "orders", label: "Orders", needsAuth: true },
            { id: "account", label: "Account", needsAuth: true },
          ].map(t => (
...
/* ---------------- HOME ---------------- */
function HomeView({ products, onOpen, onCategory, onShowSafety, onRefresh }) {
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-2 mt-1">
        ...
        <button onClick={doRefresh} disabled={refreshing} className="text-[11px] font-semibold flex items-center gap-1 px-2 py-1 rounded-lg mb-2" style={{ color: COLORS.teal, background: COLORS.paper2 }}>
          <span className={refreshing ? "animate-spin" : ""}>🔄</span> {refreshing ? "Checking…" : "Refresh"}
        </button>
      </div>
...
function ProductView({ product: p, onBack, onAddCart, onBuyNow, onMessage, onOffer, onRespondOffer, messages, profile, reviews, settings, follows, onToggleFollow, session, onOpenSeller }) {
  ...
  if (showEft) {
    ...
    const waLink = p.sellerContactPhone
      ? `https://wa.me/${p.sellerContactPhone.replace(/[^0-9]/g, "")}?text=${encodeURIComponent(`Hi, I'm interested in "${p.title}" (${money(total, p.currency)}) on THE MARKET.`)}`
      : null;
    return (
      <div>
        ...
        {bank?.accountNumber ? (
          ...
          <>
            <SectionTitle>Contact seller directly</SectionTitle>
            <div className="p-4 rounded-xl mb-3" style={{ background: "#fff", border: `1px solid ${COLORS.line}` }}>
              <div className="text-xs font-bold mb-2">{p.seller}</div>
              <div className="text-xs opacity-60 mb-3">This seller hasn't set up automatic EFT payments yet. Message them on WhatsApp to arrange payment and pickup/delivery directly.</div>
              <div style={{ borderTop: `1px solid ${COLORS.line}` }} className="pt-2 mt-2">
                <Row label="Amount" value={money(total, p.currency)} bold />
              </div>
            </div>
            <a href={waLink} target="_blank" rel="noreferrer">
              <Btn full variant="lime">Message on WhatsApp</Btn>
            </a>
            <div className="mt-2"><Btn full variant="ghost" onClick={() => onBuyNow(p, qty, delivery)}>Also track this as an order</Btn></div>
          </>
        ) : (
          <div className="p-4 rounded-xl mb-3 text-xs" style={{ background: "#fff", border: `1px solid ${COLORS.line}`, color: COLORS.rust }}>
            This seller hasn't set up payout or contact details yet — message them first using the box below.
          </div>
        )}
      </div>
    );
...
/* ---------------- SELL ---------------- */
function SellView({ onPublish, profile, onNeedProfile, actingFor, onExitActingFor }) {
  ...
  // When an admin is listing on behalf of a seller, every completeness check below
  // looks at THAT seller's stored details — never the admin's own.
  const identitySource = actingFor || profile;
  const sd = actingFor ? (actingFor.sellerDetails || {}) : (profile.sellerDetails || {});
  const sellerDetailsComplete = identitySource.sellerType === "Business" || identitySource.role === "business"
    ? !!(sd.phone && sd.businessName && sd.businessAddress)
    : !!(sd.phone && sd.address);
  const bankComplete = !!(actingFor ? actingFor.bankDetails?.accountNumber : profile.bankDetails?.accountNumber);
  const fullSetupComplete = bankComplete && sellerDetailsComplete;
  const contactPhoneValid = d.contactPhone && d.contactPhone.trim().length >= 7;
  const canPublish = fullSetupComplete || contactPhoneValid;
  const canNext = [
    !!d.category,
  ...
  return (
    <div>
      {actingFor && (
        <div className="p-2.5 rounded-xl mb-3 flex items-center justify-between text-xs" style={{ background: `${COLORS.teal}14`, border: `1px solid ${COLORS.teal}` }}>
          <span>Listing on behalf of <b>{actingFor.name}</b> — payout goes to their account, not yours.</span>
          <button onClick={onExitActingFor} className="font-semibold underline ml-2 flex-shrink-0" style={{ color: COLORS.teal }}>Cancel</button>
        </div>
      )}
      ...
      {step === 4 && (
        <div className="p-3 rounded-xl mb-3" style={{ background: "#fff", border: `1px solid ${COLORS.line}` }}>
          ...
          {actingFor && <div className="text-xs opacity-60 mt-2">Will be published as: <b>{actingFor.name}</b></div>}

          {!fullSetupComplete && (
            <div className="mt-3 pt-3" style={{ borderTop: `1px solid ${COLORS.line}` }}>
              <div className="text-xs font-semibold mb-1">{actingFor ? `${actingFor.name} hasn't finished full payout setup yet.` : "Haven't finished full payout setup yet?"}</div>
              <div className="text-[11px] opacity-60 mb-2">List anyway with just a contact number — buyers will message this number to arrange payment directly, instead of seeing the automatic EFT screen.</div>
              <input className={inputCls} style={inputStyle} value={d.contactPhone} onChange={e => setD({ ...d, contactPhone: e.target.value })} placeholder="Contact number (WhatsApp)" />
            </div>
          )}
        </div>
      )}
      ...
      <div className="flex gap-2 mt-4">
        ...
        {step === steps.length - 1 && (
          <Btn full variant="lime" disabled={!canPublish} onClick={() => {
            onPublish({ ...d, price: Number(d.price), deliveryFee: Number(d.deliveryFee) || 0, qty: Number(d.qty) || 1 }, actingFor);
          }}>
            {canPublish ? (actingFor ? `Publish for ${actingFor.name}` : "Publish listing") : "Add payout details or a contact number"}
          </Btn>
        )}
      </div>
    </div>
        ...
        {list.map(o => {
          ...
          return (
            <div key={o.id} className="p-3 rounded-xl" style={{ background: "#fff", border: `1px solid ${COLORS.line}` }}>
              ...
              {o.paymentMethod === "contact" && o.sellerContactPhone && (
                <div className="p-2 rounded-lg text-[11px] mb-2" style={{ background: COLORS.paper2 }}>
                  Arranged directly via WhatsApp · {o.sellerContactPhone}
                </div>
              )}
...
function AdminModal({ onClose, products, orders, settings, onSaveSettings, support, onSendSupport, onMarkSold, onDeleteListing, onRelist, sellers: sellerDirectory, onLoadSellers, onListForSeller }) {
  ...
  const filteredSellerDirectory = sellerDirectory.filter(s => s.name.toLowerCase().includes(sellerSearch.toLowerCase()));

  useEffect(() => { if (!sellersLoaded) { onLoadSellers(); setSellersLoaded(true); } }, [sellersLoaded, onLoadSellers]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end" style={{ background: "#00000066" }}>
      <div className="w-full rounded-t-2xl p-4 max-h-[85vh] overflow-y-auto" style={{ background: COLORS.paper }}>
        ...
        <SectionTitle>List for a seller</SectionTitle>
        <div className="p-3 rounded-xl mb-4" style={{ background: "#fff", border: `1px solid ${COLORS.line}` }}>
          <div className="text-[11px] opacity-50 mb-2">Only sellers who've already saved their own payout and seller details in Account show up here — that's what keeps their money going to them, not you.</div>
          <input className={inputCls} style={inputStyle} value={sellerSearch} onChange={e => setSellerSearch(e.target.value)} placeholder="Search by name…" />
          <div className="space-y-1 mt-2 max-h-52 overflow-y-auto">
            {filteredSellerDirectory.length === 0 && <div className="text-xs opacity-45 py-2">No sellers found. They need to sign up and complete their own Account details first.</div>}
            {filteredSellerDirectory.map(s => {
              const ready = !!(s.bankDetails?.accountNumber) && !!(s.sellerDetails?.phone);
              return (
                <div key={s.userId} className="flex items-center justify-between p-2 rounded-lg" style={{ background: COLORS.paper2 }}>
                  <div className="text-xs">
                    <div className="font-semibold">{s.name}</div>
                    {ready ? <Tag tone="lime">Ready</Tag> : <Tag tone="rust">Setup incomplete</Tag>}
                  </div>
                  <Btn small variant={ready ? "lime" : "ghost"} disabled={!ready} onClick={() => onListForSeller(s)}>
                    {ready ? "List for them" : "Not ready"}
                  </Btn>
                </div>
              );
            })}
