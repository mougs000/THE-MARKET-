...
/* ============================================================
   ...
   Display: "Archivo Black" / Body: "Inter" / Utility: "IBM Plex Mono"
   Signature: the "direct line" tag — a seller-dot—line—buyer-dot
   marker that appears on every listing, literalizing "no middlemen".
   ============================================================ */

const FONT_IMPORT = `@import url('https://fonts.googleapis.com/css2?family=Archivo+Black&family=Inter:wght@400;500;600;700&family=IBM+Plex+Mono:wght@400;500;600&display=swap');`;
...
function Btn({ children, onClick, variant = "primary", full, disabled, small }) {
  const base = "font-semibold transition active:scale-[0.98] disabled:opacity-40 disabled:active:scale-100";
  const pad = small ? "px-3 py-1.5 text-xs" : "px-4 py-3 text-sm";
  const styles = {
    primary: { background: COLORS.ink, color: COLORS.lime },
    lime: { background: COLORS.lime, color: COLORS.ink },
    ...
    rust: { background: COLORS.rust, color: "#fff" },
  };
  return (
    <button disabled={disabled} onClick={onClick}
      className={`${base} ${pad} rounded-lg ${full ? "w-full" : ""}`}
...
function AuthView({ onDone }) {
  ...
  return (
    <div style={{ background: COLORS.paper, minHeight: "100vh" }} className="flex flex-col justify-center px-6">
      <style>{FONT_IMPORT}</style>
      <div style={{ fontFamily: "'Archivo Black', sans-serif", color: COLORS.ink }} className="text-2xl mb-1 text-center">THE MARKET</div>
      <div className="text-xs text-center opacity-60 mb-6">Buy Direct. Sell Direct.</div>

      <div className="flex gap-2 mb-4">
        <button onClick={() => setMode("signin")} className="flex-1 px-3 py-2 rounded-lg text-xs font-semibold"
        ...
        <button onClick={() => setMode("signup")} className="flex-1 px-3 py-2 rounded-lg text-xs font-semibold"
...
export default function TheMarket() {
  ...
  if (session === undefined) {
    return (
      <div style={{ background: COLORS.paper, minHeight: "100vh" }} className="flex items-center justify-center">
        <style>{FONT_IMPORT}</style>
        <div style={{ fontFamily: "'Archivo Black', sans-serif", color: COLORS.ink }} className="text-xl animate-pulse">THE MARKET</div>
      </div>
    );
  }
  ...
  return (
    <div style={{ background: COLORS.paper, minHeight: "100vh", fontFamily: "'Inter', sans-serif", color: COLORS.ink }}>
      ...
      {/* header */}
      <div style={{ background: COLORS.ink }} className="sticky top-0 z-30 px-4 pt-4 pb-3">
        <div className="flex items-center justify-between">
          <div style={{ fontFamily: "'Archivo Black', sans-serif", color: COLORS.lime }} className="text-lg tracking-tight">
            THE MARKET
          </div>
          <div className="flex items-center gap-2">
            <button onClick={() => setTab("cart")} className="relative px-2 py-1 rounded-lg" style={{ background: COLORS.tealDark }}>
              ...
              {cartCount > 0 && (
                <span className="absolute -top-1.5 -right-1.5 rounded-full text-[10px] font-bold flex items-center justify-center"
                  style={{ background: COLORS.lime, color: COLORS.ink, width: 16, height: 16 }}>{cartCount}</span>
              )}
            </button>
            <button onClick={() => setTab("account")} className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold"
        ...
        {(tab === "home" || tab === "categories") && (
          <div className="mt-3 flex gap-2">
            <input value={query} onChange={e => setQuery(e.target.value)} placeholder="What are you looking for?"
              className="flex-1 px-3 py-2.5 rounded-lg text-sm outline-none" style={{ background: "#fff" }} />
          </div>
        )}
      </div>
      ...
      {/* bottom nav */}
      <div className="fixed bottom-0 left-0 right-0 z-30" style={{ background: "#fff", borderTop: `1px solid ${COLORS.line}` }}>
        <div className="flex items-stretch justify-around px-1 py-2">
          {[
            ...
            <button key={t.id} onClick={() => setTab(t.id)}
              ...
              style={t.highlight ? { background: COLORS.lime } : {}}>
              <span className="text-base">{t.icon}</span>
              <span className="text-[10px] font-semibold" style={{ color: tab === t.id ? COLORS.teal : COLORS.ink, opacity: tab === t.id || t.highlight ? 1 : 0.55 }}>{t.label}</span>
            </button>
          ))}
        </div>
      </div>
...
function HomeView({ products, onOpen, onCategory }) {
  ...
  return (
    <div>
      <div className="mb-5 p-4 rounded-xl" style={{ background: COLORS.teal, color: "#fff" }}>
        <div style={{ fontFamily: "'Archivo Black', sans-serif" }} className="text-lg leading-tight mb-1">Buy Direct.<br />Sell Direct.</div>
        <div className="text-xs opacity-80 mb-2">Every listing connects you straight to the supplier — no unnecessary middlemen.</div>
        <DirectLine />
      </div>
      ...
      <div className="grid grid-cols-4 gap-2 mb-6">
        {CATEGORIES.slice(0, 8).map(c => (
          <button key={c.id} onClick={() => onCategory(c.id)} className="flex flex-col items-center gap-1 p-2 rounded-lg"
...
function ProductGrid({ products, onOpen }) {
  ...
  return (
    <div className="grid grid-cols-2 gap-3 mb-2">
      {products.map(p => (
        <button key={p.id} onClick={() => onOpen(p)} className="text-left p-3 rounded-xl" style={{ background: "#fff", border: `1px solid ${COLORS.line}` }}>
      ...
      {step === 2 && (
        <div>
          ...
          <Field label="Product video (optional)">
            <input className={inputCls} style={inputStyle} value={d.videoUrl} onChange={e => setD({ ...d, videoUrl: e.target.value })} placeholder="https://…mp4" />
            <div className="text-[11px] opacity-45 mt-1">Link to a hosted video file. Buyers can play it directly on the product page.</div>
          </Field>
    ...
    <div className="fixed inset-0 z-50 flex items-end" style={{ background: "#00000066" }}>
      <div className="w-full rounded-t-2xl p-4 max-h-[85vh] overflow-y-auto" style={{ background: COLORS.paper }}>
        <div className="flex justify-between items-center mb-3">
          <div style={{ fontFamily: "'Archivo Black', sans-serif" }} className="text-base">Admin</div>
